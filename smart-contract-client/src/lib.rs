...
impl ComputeManagerClient {
    ...
    pub async fn submit_openrank_tx(&self, tx: Tx) -> Result<()> {
        ...
        let is_tx_exists = match tx.body() {
            tx::Body::ComputeCommitment(_) | tx::Body::ComputeVerification(_) => {
        ...
        let _result_hash = match tx.body() {
            tx::Body::ComputeCommitment(compute_commitment) => {
            ...
            tx::Body::ComputeVerification(compute_verification) => {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        for tx in txs {
            ...
            info!("Posted TX on chain: {:?}", tx.body().prefix());
        }
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_submit_openrank_tx() -> Result<()> {
        ...
        client
            .submit_openrank_tx(Tx::default_with(tx::Body::ComputeRequest(
        ...
        let mut tx = Tx::default_with(tx::Body::ComputeCommitment(compute::Commitment::new(
            TxHash::from_bytes(
        ...
        let mut tx = Tx::default_with(tx::Body::ComputeVerification(compute::Verification {
