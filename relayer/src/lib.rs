    ...
    pub async fn init(db_configs: Vec<(&str, &str)>) -> Self {
        ...
        for (db_path, topic) in db_configs {
            let last_processed_key = target_db
                .load_last_processed_key(&format!("relayer_last_key_{}_{}", db_path, topic)).await
    ...
    async fn index(&mut self) {
        for (db_path, (db, topic)) in &self.dbs {
            ...
            debug!(
                ...
                db_path,
                topic,
                self.last_processed_keys[db_path]
            );
            ...
            for item in iter {
                match item {
                    Ok((key, value)) => {
                        ...
                        if
                            self.last_processed_keys[db_path].is_none() ||
                            self.last_processed_keys[db_path].unwrap() < count
                        {
                            let tx: Tx = serde_json
                                ::from_str(value_str.as_ref())
                    ...
                    Err(e) => {
                        error!("Error iterating over DB {} and topic {}: {}", db_path, topic, e);
                    }
                }
                count += 1;
            }

            info!("Total records processed for {} and topic {}: {}", db_path, topic, count);
        }
    }
